<template>
  <div class="novelMenuList">
    ...
    <div class="menuBottom">
      <div @click="NovelMenuList(novelId,prePages)">上一页</div>
      <div @click="NovelMenuList(novelId,lastPages)">下一页</div>
    </div>
  </div>
</template>
<script>
    ...
    methods: {
      ...
      NovelMenuList(id,page){
        ...
        this.$http({
          ...
          params:{id:id,begin:page}
        }).then(res=>{
          if(res.status==200){
            console.log(res);
            this.menuLists = res.data.catalogList;
            // this.prePages = this.lastPages;
            // this.lastPages = res.data.catalogList[res.data.catalogList.length-1].chapter;
          }
        }).catch();
      },
